...
pub struct NetstatLinux;

impl Module for NetstatLinux {
    fn name(&self) -> &'static str { "linux/netstat" }
    fn description(&self) -> &'static str { "Show network connections (netstat -tunap)" }
    fn usage(&self) -> &'static str { "Usage: run-module linux/netstat <session_id>" }
    ...
    fn run(&self, session_id: usize, session_manager: &crate::core::session::SessionManager, args: Vec<String>) {
        if let Some(session) = session_manager.get(session_id) {
            ...
            let cmd = format!("netstat -tunap; echo {}\n", marker);
